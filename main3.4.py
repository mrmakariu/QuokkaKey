...
PRODUCTS = {
    ...
    'telegram_premium': [
        ...
        {'name': '1 год', 'price': 2359, 'pay_url': 'https://pay.cloudtips.ru/p/418d424e'}
    ]
}
...
PRODUCT_DESCRIPTIONS = {
    'spotify': {
        'Индивидуальная': "Spotify Индивидуальная 1 месяц\n🎧 Персональный аккаунт с музыкой без рекламы, с возможностью скачивать треки и слушать их в любом порядке с высоким качеством звука.\n\n✨ Если у Вас нету аккаунта данного сервиса, то Вы можете приобрести его, обратившись в поддержку - @quokka_key",
        'Дуо': "Spotify Дуо 1 месяц\n🎶 Два отдельных Premium-аккаунта, экономичнее двух индивидуальных подписок.\n\n✨ Если у Вас нету аккаунта данного сервиса, то Вы можете приобрести его, обратившись в поддержку - @quokka_key",
        'Семейная': "Spotify Семейная 1 месяц\n🎼 До шести отдельных Premium-аккаунтов, с раздельной персонализацией и рекомендациями для каждого.\n\n✨ Если у Вас нету аккаунта данного сервиса, то Вы можете приобретить его, обратившись в поддержку - @quokka_key"
    },
    'netflix': {
        'Basic': "Netflix Basic 1 месяц\n🎬 Доступ к библиотеке фильмов и сериалов с возможностью просмотра на одном устройстве, часто включает рекламу и контент в стандартном качестве.\n⚠️ VPN необходим для использования сервиса в России, без него доступ невозможен! 🚨\n\n✨ Если у Вас нету аккаунта данного сервиса, то Вы можете приобрести его, обратившись в поддержку - @quokka_key",
        'Standard': "Netflix Стандарт 1 месяц\n🍿 Просмотр в Full HD качестве на двух устройствах одновременно с возможностью добавления одного дополнительного пользователя и скачивания контента.\n⚠️ VPN необходим для использования сервиса в России, без него доступ невозможен! 🚨\n\n✨ Если у Вас нету аккаунта данного сервиса, то Вы можете приобрести его, обратившись в поддержку - @quokka_key",
        'Premium': "Netflix Premium 1 месяц\n🎥 Просмотр на четырех устройствах одновременно в качестве Ultra HD 4K с возможностью добавления двух дополнительных пользователей и пространственным аудио.\n⚠️ VPN необходим для использования сервиса в России, без него доступ невозможен! 🚨\n\n✨ Если у Вас нету аккаунта данного сервиса, то Вы можете приобрести его, обратившись в поддержку - @quokka_key"
    },
    'adobe': {
        'Photoshop': "Photoshop 1 месяц\n📷 Профессиональный инструмент для обработки фотографий, создания иллюстраций, макетов и прототипов для веб-дизайна.\n\n✨ Если у Вас нету аккаунта данного сервиса, то Вы можете приобрести его, обратившись в поддержку - @quokka_key",
        'After Effects': "After Effects 1 месяц\n✨ Продвинутый редактор для создания впечатляющих визуальных эффектов, анимации и графических композиций для фильмов, рекламы и веб-проектов.\n\n✨ Если у Вас нету аккаунта данного сервиса, то Вы можете приобрести его, обратившись в поддержку - @quokka_key",
        'Premiere Pro': "Premiere Pro 1 месяц\n🎞️ Мощная программа для монтажа видео с функциями нарезки и склейки фрагментов, цветокоррекции и профессиональной работы со звуком.\n\n✨ Если у Вас нету аккаунта данного сервиса, то Вы можете приобрести его, обратившись в поддержку - @quokka_key"
    },
    'telegram_premium': {
...
# ИСПРАВЛЕНО: Добавлен `state='*'` и `state: FSMContext`, чтобы кнопка работала из любого состояния.
@dp.callback_query_handler(lambda c: c.data.startswith('admin_done_'), state='*')
async def admin_done(cb: types.CallbackQuery, state: FSMContext):
    await state.finish()  # Завершаем любое возможное состояние модератора
...
# ИСПРАВЛЕНО: Добавлен `state='*'`, чтобы кнопка работала из любого состояния.
@dp.callback_query_handler(lambda c: c.data.startswith('admin_decline_'), state='*')
...
async def process_decline_reason(msg: types.Message, state: FSMContext):
    ...
    await bot.send_message(
        user_id,
        f"К сожалению, ваш заказ #{oid} был отклонён. Причина: {reason}\n\nЕсли есть вопросы — пишите в поддержку @quokka_key"
    )
    
    # Редактируем сообщение в чате модераторов
    # Для этого нужно найти исходное сообщение. Это сложная задача без сохранения message_id.
    # Простым решением будет просто ответить в чат модераторов.
    await msg.answer(f"Заказ #{oid} отклонен. Причина отправлена пользователю.", reply_markup=back_to_start_kb())
